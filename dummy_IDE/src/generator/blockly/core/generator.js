...
Blockly.Generator.prototype.blockToCode = function (block, opt_thisOnly) {
    ...
    generation.nest--;
    // Attempting to fix higlighting bug. Editors were passed as null current id never changed 
    generation.currentSystemEditorId = window.workspace["blockly1"].getBlockById(block.id) ? "blockly1" : "blockly2";
    if (Array.isArray(code)) {
        ...
        let id = block.id.replace(/\$/g, '$$$$');  // Issue 251.
        let cs = genEval(my_nest, block.id, generation.currentSystemEditorId);
        if (this.STATEMENT_PREFIX && !block.suppressPrefixSuffix) {
            //code = this.injectId(this.STATEMENT_PREFIX, cs) + code;
            code = this.STATEMENT_PREFIX.replace(/%1/g, cs) + code;
        }
